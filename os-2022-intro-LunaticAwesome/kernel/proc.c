...
int dump(void) {
  ...
  int regs[10] = {frame->s2, frame->s3, frame->s4, frame->s5,  frame->s6,
                  frame->s7, frame->s8, frame->s9, frame->s10, frame->s11};
...
int dump2(int pid, int register_num, uint64 *return_value) {
  ...
  struct proc *p = myproc();
  for (struct proc *child_proc = proc; child_proc < &proc[NPROC];
       ++child_proc) {
    if (child_proc->pid == pid) {
      if (child_proc->parent != p && child_proc != p) {
        return -1;
      }
      if (child_proc->state == ZOMBIE) {
        return -2;
      }
      struct trapframe *frame = child_proc->trapframe;
      uint64 regs[10] = {frame->s2,  frame->s3, frame->s4, frame->s5,
                         frame->s6,  frame->s7, frame->s8, frame->s9,
                         frame->s10, frame->s11};
      if (copyout(myproc()->pagetable, *return_value,
                  (char *)(regs + register_num - 2), sizeof(regs[0])) != 0) {
        return -4;
      }
      return 0;
    }
