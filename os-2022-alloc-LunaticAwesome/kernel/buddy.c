...
void alloc_bit_xor(char *array, int index) {
  index /= 2;
  char b = array[index / 8];
  char m = (1 << (index % 8));
  array[index / 8] = (b ^ m);
...
void alloc_bit_set(char *array, int index) { bit_set(array, index / 2); }

void alloc_bit_clear(char *array, int index) { bit_clear(array, index / 2); }

int alloc_bit_isset(char *array, int index) {
  return bit_isset(array, index / 2);
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  alloc_bit_xor(bd_sizes[k].alloc, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    alloc_bit_set(bd_sizes[k - 1].alloc, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    alloc_bit_xor(bd_sizes[k].alloc, bi);             // free p at size k
    if (alloc_bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      alloc_bit_xor(bd_sizes[k].alloc, bi);
    }
  }
}
...
int bd_initfree_pair(int k, int bi, int flag_right) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (alloc_bit_isset(bd_sizes[k].alloc, bi) == 1) {
