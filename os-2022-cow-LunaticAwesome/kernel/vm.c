...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len,
  ...
  while (len > 0) {
    ...
    if (pa0 == 0) {
      pa0 = (uint64)lazyalloc(pagetable, va0);
    }
    n = PGSIZE - (dstva - va0);
    if (n > len) n = len;
...
void printwalk(pagetable_t pagetable, int lvl) {
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte == 0) {
      continue;
    ...
    printf("%d: pte %p pa %p\n", i, pte, child);
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) {
      printwalk((pagetable_t)child, lvl + 1);
    }
  }
}
...
void *cowalloc(pagetable_t pagetable, uint64 va) {
  ...
  if (get_ref(pa) == 1) {
    ...
    *pte &= ~PTE_F;
    ...
    int flags = (PTE_FLAGS(*pte) | PTE_W) & ~PTE_F;
...
void *lazyalloc(pagetable_t pagetable, uint64 va) {
  ...
  pte_t *pte = walk(pagetable, va, 0);
  if (pte != 0 && (*pte & (PTE_R | PTE_W | PTE_X | PTE_U))) {
    return 0;
  }
  char *mem = kalloc();
  if (mem == 0) {
